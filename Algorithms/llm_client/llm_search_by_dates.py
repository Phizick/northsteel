...
async def llm_search_by_dates(dates, credentials, company):
    ...
    f"Каждый объект должен иметь следующие поля: 'Компания'{company}, одно значение из {credentials}, "
                            f"и по одному полю для каждого из указанных годов с префиксом Итого. "
    f"Убедитесь, что данные структурированы правильно, и каждый год для каждой компании представлен в "
                            f"отдельном объекте. массив ответови без имени")
    }
    ],
        "max_tokens": 4096
    }
