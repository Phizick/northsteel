...
async def llm_text_parser(data):
    ...
    data = {
        "messages": [
            {
                "role": "user",
                "content": f"Пожалуйста расскажи подробно на русском языке о компании {data}. ответ верни в виде обьекта JSON Следи за тем, что бы не было вложенных обьектов/массивов и формат данных сохранялся четко"
